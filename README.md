...
```
GLM-5.1 | ▕████▍     ▏44% 56.7K/128.0K Compact×2 | In:2.4M Out:10.7K Cache:2.2M Think:952 | Req:29 | Cost:$0.023 | Credits:67.20 | Time:45s | +156/-23
...
| `Compact×2` | Context 压缩次数（仅压缩 > 0 时显示） |
...
子 Agent 的 `running_agents` 和 `compact_count` 不计入主统计（前者由主 transcript 的 Agent 调用/返回追踪，后者仅统计主 context 的压缩事件）。子 Agent 的工具调用、token 用量、Credits 则会合并。
...
3. 完整版脚本增量读取 `transcript_path` 指向的 JSONL 文件，解析 `function_call` 条目统计工具调用，解析 `providerData.usage` / `providerData.rawUsage` 获取 Token 明细（含 `cacheReadInputTokens`、`cacheWriteOutputTokens`）
...
涵盖增量解析、子 Agent 聚合、Compact 计数、null 安全、旧缓存兼容、文件截断处理、崩溃安全、缓存语义、自动更新等场景。
