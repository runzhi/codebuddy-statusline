...
<span style="opacity:0.6">Recent:</span> <span style="color:#55ff55">In:</span>65.0K <span style="color:#55ff55">Out:</span>90 <span style="opacity:0.6">Cache:</span>64.8K(99%) <span style="color:#ff5555">Cost:</span>$0.02(¥0.11) <span style="color:#ffff55">Credits:</span>1.56 | <span style="color:#55ffff">Bash</span> <span style="opacity:0.6">rm /tmp/iwiki_full.md /tmp/save_iwiki.py 2&gt;/dev/null; echo …</span> | <span style="color:#55ffff">Bash</span> <span style="opacity:0.6">cat &gt; /tmp/save_iwiki.py &lt;&lt; 'PYEOF'</span>
</pre>
...
| `Req:29` | API 请求次数 |
| `Cost:$0.69(¥4.83)` | 费用，美元(人民币)。总费用 = 平台 cost + credits/100，人民币 = 美元×7。红色显示 |
...
Git-clone 安装模式下， `statusline.py` 内置了**每天最多一次**的 `git pull` 自动更新机制：
...
## ~/.codebuddy/settings.json(脚本自动写入)
...
`statusline.py` 经过精心优化，每次调用的开销极低：
...
缓存按 session_id 隔离，存储在 `~/.codebuddy/plugins/data/statusline/cache/`，超过 7 天自动清理。

### 子 Agent Token 统计

自动扫描 `subagents/` 目录下的子 Agent transcript 文件，将其 token 用量合并到主统计中。这样状态栏显示的 In/Out/Cache/Credits 包含了所有子 Agent 的消耗，而不仅仅是主 Agent 的。
...
3. 脚本增量读取 `transcript_path` 指向的 JSONL 文件，解析 `function_call` 条目统计工具调用，解析 `providerData.usage` / `providerData.rawUsage` 获取 Token 明细（Cache 来自 `inputTokensDetails[].cached_tokens`，并兼容 `prompt_cache_hit_tokens`），解析 `summary` 条目区分 `pre-compact`（Auto-Compact×N）和 `periodic`（Periodic×M）
...
├── statusline.py            # statusline 脚本（增量解析 + 子 Agent 聚合 + 自动更新）
...
涵盖增量解析、子 Agent 聚合、AutoCompact 计数、null 安全、旧缓存兼容、文件截断处理、崩溃安全、缓存语义、自动更新等场景。
