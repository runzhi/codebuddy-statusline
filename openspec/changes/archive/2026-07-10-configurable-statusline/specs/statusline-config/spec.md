...
- **Then** all blocks are shown in the built-in canonical order and both Tools
  and Recent lines are enabled, producing output identical to the pre-config build.
...
The plugin MUST provide a `/statusline` command backed by a `config.py` helper
